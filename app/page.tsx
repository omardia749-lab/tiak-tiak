      ...
      async (pos) => {
        ...
        if (currentDriverRide) {
          if (driverPhase === 'to_client') {
            ...
            // Détection d'arrêt anormal (5 min hors destination)
if (currentDriverRide && driverPhase === 'with_client') {
  const now = Date.now()
  if (!lastMovementRef.current) lastMovementRef.current = now
  const distMoved = haversineDistance(latitude, longitude, lastPosRef.current?.lat || latitude, lastPosRef.current?.lng || longitude) * 1000
  if (distMoved > 20) {
    lastMovementRef.current = now
    lastPosRef.current = { lat: latitude, lng: longitude }
  } else if (now - lastMovementRef.current > 5 * 60 * 1000) {
    // Arrêt de plus de 5 minutes — alerte
    await fetch('/api/sos', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        ride_id: currentDriverRide.id,
        triggered_by: 'systeme',
        triggered_by_name: 'Arrêt anormal détecté',
        ...
        lat: latitude,
        lng: longitude,
        address: `Arrêt anormal — immobile depuis 5 min`,
      }),
    })
    lastMovementRef.current = now // reset pour éviter spam
  }
}
  ...
  if (distMoved > 20) {
    ...
    lastMovementRef.current = now
            ...
            if (routeCoords && routeCoords.length > 0) {
              ...
              if (minDist > 500) {
                // Déclencher le protocole silencieux
                await fetch('/api/sos', {
                  method: 'POST',
                  headers: { 'Content-Type': 'application/json' },
                  body: JSON.stringify({
                    ride_id: currentDriverRide.id,
                    triggered_by: 'systeme',
                    triggered_by_name: 'Détection automatique',
                    triggered_by_phone: user.phone || '',
                    other_party_name: currentDriverRide.client_name || '',
                    other_party_phone: currentDriverRide.client_phone || '',
                    lat: latitude,
                    lng: longitude,
                    address: `Déviation détectée — ${Math.round(minDist)}m du tracé`,
                  }),
                })
              }
            }
...
const distToPickup = haversineDistance(driverPosition.lat, driverPosition.lng, newRide.from_lat, newRide.from_lng)
 ...
 const signupDriver = async () => {
  ...
  const commanderCourse = async () => {
    ...
    const { data: blackcheck } = await fetch(`/api/blacklist?phone=${user.phone}`).then(r => r.json()).catch(() => ({ blacklisted: false }))
